...
Run from the repo root with the venv active:
    ...
    python -m pdvp.core.test.test_inspect
"""
import importlib
import sys

inspection = importlib.import_module("pdvp.core.commands.inspection")
from pdvp.core.model import CompletionList, Error, ExceptionInfo, Scope, Status
...
def test_exception_info_dap_error_path_still_returns_a_clean_error() -> None:
    ...
    from pdvp.core.model import ErrorKind, PydevdRefused
    session, client, restore = _new_session_env()
    try:
        _stopped_at_a_frame(session, client)
