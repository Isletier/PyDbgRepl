...
Run from the repo root with the venv active:
    ...
    python -m pdvp.core.test.test_config
"""
import dataclasses

from pdvp.core import launch
# Aliased: several tests use `config` as a local name for a Config instance.
from pdvp.core import config as _config
from pdvp.core.config import CONFIG, Config, LaunchError, LogLevel, QtSupport, VmType
...
def test_normalize_rejects_wrong_types() -> None:
    c = Config()
    c.module = 2
    _expect_error(lambda: _config.normalize(c), "module")
...
def test_del_restores_one_field() -> None:
    ...
    config.completion = "classical"

    del config.log_level
    assert config.log_level is LogLevel.CRITICAL
    assert config.completion == "classical"  # neighbours untouched
...
def test_reset_restores_everything_in_place() -> None:
    ...
    assert config.log_level is LogLevel.CRITICAL
    assert config.completion == "debugger"
...
def test_reset_keeps_the_object_identity() -> None:
    ...
    `pdvp.CONFIG` and `pdvp.core.config.CONFIG`. reset() must therefore
