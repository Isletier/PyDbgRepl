...
Two access patterns, matched to what each test actually needs to prove:
  ...
  * `real_run()` (below), which drives the real command layer against the
    real `pdvp.session.SESSION` singleton via `pdvp.commands.lifecycle.run()`
...
`SESSION`/`CONFIG` are process-wide singletons, unlike `pdvp/test/test_execution.py`'s
...
Run from the repo root with the venv active:
    ...
    python -m pdvp.dap.test.test_multithreading
"""
import contextlib
import importlib
import os
